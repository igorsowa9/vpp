...
class VPP_ext_agent(Agent):
    ...
    def similarity(self, t):
        ...
        # Load Memory
        path_memory = '/home/iso/Desktop/vpp_some_results/2018_0822_1555_week1-2/'
        path_memory_pickle = path_memory + "temp_ln_" + str(my_idx) + ".pkl"
        f = pd.read_pickle(path_memory_pickle)
        ...
        label_ranges = {"with_idx_req": np.round(
            np.abs(learn_memory_mod['with_idx_req'].max() - learn_memory_mod['with_idx_req'].min()), 4),
                        "minute_t": 24 * 60 / 2,
                        "week_t": 7 / 2,
                        "month_t": 12 / 2,
                        "av_weather": np.round(
                            np.abs(learn_memory_mod['av_weather'].max() - learn_memory_mod['av_weather'].min()), 4)}
        ...
        for vpp_idx in po_idx:
            ...
            for idx in range(vpp_file['bus_n']):
                ...
                if not gen_path == "":
                    ...
                    forecast_max_generation[idx] = np.round(forecast_max_generation_factor[idx] * max_generation0[idx],
                                                            4)
                po_wf.update({vpp_idx: forecast_max_generation})
